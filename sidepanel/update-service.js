(() => {
  const GITHUB_OWNER = 'a519592965';
  const GITHUB_REPO = 'FlowGate';
  ...
  const VERSION_FAMILY_FLOWGATE = 'flowgate';
  ...
  function getVersionFamily(version, fallbackFamily = VERSION_FAMILY_LEGACY) {
    ...
    if (/^flowgate/i.test(trimmed)) {
      return VERSION_FAMILY_FLOWGATE;
    }
  ...
  function stripVersionPrefix(version) {
    return String(version || '').trim().replace(/^(?:(?:flowgate|ultra|pro)\s*v?|v)\s*/i, '');
  }
  ...
  function getVersionFamilyPrefix(family) {
    if (family === VERSION_FAMILY_FLOWGATE) {
      return 'FlowGate ';
    }
  ...
  function getComparableReleaseVersion(release) {
    const fallbackFamily = [VERSION_FAMILY_FLOWGATE, VERSION_FAMILY_ULTRA, VERSION_FAMILY_PRO].includes(release?.family)
      ? release.family
  ...
  function setIgnoredUpdateVersion(version) {
    const normalized = formatDisplayVersion(version, VERSION_FAMILY_FLOWGATE) || String(version || '').trim();
    try {
      if (normalized) {
  ...
  function getLocalVersionLabel(manifest = chrome.runtime.getManifest()) {
    const versionName = formatDisplayVersion(manifest?.version_name, VERSION_FAMILY_FLOWGATE);
    if (versionName) {
      return versionName;
    }

    const versionCore = extractVersionCore(manifest?.version || '');
    return versionCore ? formatDisplayVersion(`FlowGate ${versionCore}`, VERSION_FAMILY_FLOWGATE) : '';
  }

  async function getReleaseSnapshot(options = {}) {
    const localVersion = getLocalVersionLabel(chrome.runtime.getManifest()) || 'FlowGate 0.0';
