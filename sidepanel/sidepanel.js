...
const rowPageScopedProxy = document.getElementById('row-page-scoped-proxy');
const inputPageScopedProxyEnabled = document.getElementById('input-page-scoped-proxy-enabled');
const inputPageScopedProxyUrl = document.getElementById('input-page-scoped-proxy-url');
const rowPageScopedProxyTest = document.getElementById('row-page-scoped-proxy-test');
const btnPageScopedProxyTest = document.getElementById('btn-page-scoped-proxy-test');
...
const displayPageScopedProxyTestResult = document.getElementById('display-page-scoped-proxy-test-result');
...
const GUIDE_REPOSITORY_URL = 'https://github.com/a519592965/FlowGate';
...
function normalizePageScopedProxyUrlValue(value = '') {
  return normalizePlusCheckoutConversionProxyUrlValue(value);
}
...
function collectSettingsPayload() {
  ...
  return {
    ...
    plusCheckoutConversionProxyUrl: typeof inputPlusCheckoutConversionProxy !== 'undefined' && inputPlusCheckoutConversionProxy
      ...
      : '',
    pageScopedProxyEnabled: Boolean(inputPageScopedProxyEnabled?.checked),
    pageScopedProxyUrl: typeof inputPageScopedProxyUrl !== 'undefined' && inputPageScopedProxyUrl
      ? normalizePageScopedProxyUrlValue(inputPageScopedProxyUrl.value)
      : '',
...
function applySettingsState(state) {
  ...
  if (typeof inputPageScopedProxyEnabled !== 'undefined' && inputPageScopedProxyEnabled) {
    inputPageScopedProxyEnabled.checked = Boolean(state?.pageScopedProxyEnabled);
  }
  if (typeof inputPageScopedProxyUrl !== 'undefined' && inputPageScopedProxyUrl) {
    inputPageScopedProxyUrl.value = normalizePageScopedProxyUrlValue(state?.pageScopedProxyUrl || '');
  }
  updatePlusCheckoutConversionModeUi();
...
function renderUpdateReleaseList(releases = []) {
  ...
  releases.forEach((release) => {
    ...
    version.textContent = release.displayVersion || `FlowGate ${release.version}`;
...
async function initializeReleaseInfo() {
  ...
  const localVersion = sidepanelUpdateService?.getLocalVersionLabel?.(chrome.runtime.getManifest())
    ...
    || (chrome.runtime.getManifest()?.version ? `FlowGate ${chrome.runtime.getManifest().version}` : '');
...
function setPageScopedProxyTestResult(message = '未测试', options = {}) {
  if (!displayPageScopedProxyTestResult) {
    return;
  }
  const normalizedMessage = String(message || '').trim() || '未测试';
  const status = String(options?.status || 'idle').trim().toLowerCase();
  const detail = String(options?.detail || '').trim();
  displayPageScopedProxyTestResult.textContent = normalizedMessage;
  displayPageScopedProxyTestResult.title = detail || normalizedMessage;
  displayPageScopedProxyTestResult.classList.remove('status-running', 'status-success', 'status-error');
  if (status === 'running') {
    displayPageScopedProxyTestResult.classList.add('status-running');
  } else if (status === 'success') {
    displayPageScopedProxyTestResult.classList.add('status-success');
  } else if (status === 'error') {
    displayPageScopedProxyTestResult.classList.add('status-error');
  }
}
...
function updatePlusCheckoutConversionModeUi() {
  ...
  const plusModeEnabledForPageProxy = plusModeEnabled;
  const pageScopedProxyLocked = Boolean(inputPageScopedProxyEnabled?.checked);
  if (inputPageScopedProxyEnabled) {
    inputPageScopedProxyEnabled.disabled = !plusModeEnabledForPageProxy;
  }
  if (inputPageScopedProxyUrl) {
    inputPageScopedProxyUrl.disabled = !plusModeEnabledForPageProxy || pageScopedProxyLocked;
    inputPageScopedProxyUrl.readOnly = pageScopedProxyLocked;
    inputPageScopedProxyUrl.setAttribute('aria-disabled', inputPageScopedProxyUrl.disabled ? 'true' : 'false');
    inputPageScopedProxyUrl.title = pageScopedProxyLocked
      ? '流程页面代理已启用，当前输入框已锁定。'
      : '除第 6 步云端支付转换外，流程页面执行期间临时生效；留空则沿用当前网络环境。';
  }
  if (btnPageScopedProxyTest) {
    btnPageScopedProxyTest.disabled = !plusModeEnabledForPageProxy || pageScopedProxyLocked;
    btnPageScopedProxyTest.setAttribute('aria-disabled', btnPageScopedProxyTest.disabled ? 'true' : 'false');
    btnPageScopedProxyTest.title = pageScopedProxyLocked ? '流程页面代理已启用，当前无法测试。' : '';
  }
  if (rowPageScopedProxy) rowPageScopedProxy.style.display = plusModeEnabledForPageProxy ? '' : 'none';
  if (rowPageScopedProxyTest) rowPageScopedProxyTest.style.display = plusModeEnabledForPageProxy ? '' : 'none';
}
...
async function handlePageScopedProxyTest() {
  if (!btnPageScopedProxyTest || !inputPageScopedProxyUrl) {
    return;
  }

  const proxyUrl = normalizePageScopedProxyUrlValue(inputPageScopedProxyUrl.value);
  inputPageScopedProxyUrl.value = proxyUrl;
  if (!proxyUrl) {
    setPageScopedProxyTestResult('请先填写代理', {
      status: 'error',
      detail: '请先填写流程页面代理地址，再执行测试。',
    });
    showToast('请先填写流程页面代理地址。', 'error');
    return;
  }

  const previousLabel = btnPageScopedProxyTest.textContent;
  btnPageScopedProxyTest.disabled = true;
  btnPageScopedProxyTest.textContent = '测试中...';
  setPageScopedProxyTestResult('测试中...', {
    status: 'running',
    detail: '正在检测代理连通性。',
  });
  ...
  try {
    ...
    const exitSummary = exitIp
      ? `${exitIp}${exitRegion ? ` [${exitRegion}]` : ''}`
      : '已连接';
    setPageScopedProxyTestResult(`可用: ${exitSummary}`, {
      status: 'success',
      detail: response?.diagnostics || exitSummary,
    });
    showToast(`流程页面代理测试通过：${exitSummary}`, 'success', 2500);
  } catch (error) {
    const message = error?.message || String(error || '流程页面代理测试失败');
    setPageScopedProxyTestResult('测试失败', {
      status: 'error',
      detail: message,
    });
    showToast(message, 'error');
  } finally {
    btnPageScopedProxyTest.disabled = Boolean(inputPageScopedProxyEnabled?.checked) || !Boolean(inputPlusModeEnabled?.checked);
    btnPageScopedProxyTest.textContent = previousLabel || '测试流程代理';
  }
}
...
inputPageScopedProxyEnabled?.addEventListener('change', () => {
  updatePlusCheckoutConversionModeUi();
  setPageScopedProxyTestResult(inputPageScopedProxyEnabled.checked ? '已启用' : '未测试');
  saveSettings({ silent: true }).catch(() => { });
});
inputPageScopedProxyUrl?.addEventListener('input', () => {
  setPageScopedProxyTestResult('未测试');
  scheduleSettingsAutoSave();
});
inputPageScopedProxyUrl?.addEventListener('blur', () => {
  inputPageScopedProxyUrl.value = normalizePageScopedProxyUrlValue(inputPageScopedProxyUrl.value);
  setPageScopedProxyTestResult('未测试');
  saveSettings({ silent: true }).catch(() => { });
});
btnPageScopedProxyTest?.addEventListener('click', () => {
  handlePageScopedProxyTest().catch((error) => {
    showToast(error?.message || String(error || '流程页面代理测试失败'), 'error');
  });
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.pageScopedProxyEnabled !== undefined && inputPageScopedProxyEnabled) {
        inputPageScopedProxyEnabled.checked = Boolean(message.payload.pageScopedProxyEnabled);
        updatePlusCheckoutConversionModeUi();
      }
      if (message.payload.pageScopedProxyUrl !== undefined && inputPageScopedProxyUrl) {
        inputPageScopedProxyUrl.value = normalizePageScopedProxyUrlValue(message.payload.pageScopedProxyUrl);
        updatePlusCheckoutConversionModeUi();
      }
