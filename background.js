...
const BUILTIN_PLUS_CHECKOUT_CLOUD_CONVERSION_API_URL = 'https://checkout.kcsy1.com/api/checkout';
const BUILTIN_PLUS_CHECKOUT_CLOUD_CONVERSION_API_KEY = 'd9fe4b496849743152c9616b8a14401d2eac779463f0e6ca395bf53ca164b8c8';
  ...
  return withIcloudLoginHelp('获取 iCloud 隐私邮箱', async () => {
    ...
    try {
      ...
      const reserveData = {
        ...
        note: 'Generated through FlowGate',
      };
...
async function executeNodeViaCompletionSignal(nodeId, timeoutMs = 0) {
  ...
  if (shouldUsePageScopedProxyForNode(normalizedNodeId, executionState) && pageScopedProxyDepth === 0) {
    return withPageScopedProxyForNode(normalizedNodeId, executionState, () => executeNodeViaCompletionSignal(nodeId, timeoutMs));
  }
...
let pageScopedProxyFlowSnapshot = null;

function normalizePageScopedProxyProtocol(value = '') {
  const normalized = String(value || '').trim().toLowerCase();
...
function normalizePageScopedProxyPort(value = '') {
  const numeric = Number.parseInt(String(value || '').trim(), 10);
  return Number.isInteger(numeric) && numeric > 0 && numeric <= 65535 ? numeric : 0;
}

function parsePageScopedProxyUrl(value = '') {
  const rawValue = String(value || '').trim();
  if (!rawValue) {
    return null;
  }
  ...
  try {
    ...
    throw new Error('流程页面代理不是有效 URL，请填写 http://host:port 或 socks5h://user:pass@host:port。');
  }
  const protocol = normalizePageScopedProxyProtocol(String(parsed.protocol || '').replace(/:$/g, ''));
  if (!protocol) {
    throw new Error('流程页面代理仅支持 http / https / socks4 / socks5 / socks5h。');
  }
  const host = String(parsed.hostname || '').trim();
  const port = normalizePageScopedProxyPort(parsed.port);
  if (!host || !port) {
    throw new Error('流程页面代理缺少有效 Host 或 Port。');
  }
  return {
    protocol,
    host,
...
function describePageScopedProxyEntry(entry = null) {
...
function buildPageScopedFixedProxyConfig(entry = null) {
  ...
  return {
    ...
    rules: {
      singleProxy: {
        scheme: String(entry.protocol || '').trim().toLowerCase() === 'socks5h' ? 'socks5' : String(entry.protocol || '').trim().toLowerCase(),
        host: entry.host,
        port: entry.port,
      },
      bypassList: PAGE_SCOPED_PROXY_BYPASS_LIST.slice(),
    },
  };
}

function getPageScopedChromeProxySettings(details = { incognito: false }) {
...
function setPageScopedChromeProxySettings(value) {
  const proxySettings = chrome?.proxy?.settings;
  ...
  return new Promise((resolve, reject) => {
    proxySettings.set({
      value,
      scope: PAGE_SCOPED_PROXY_SETTINGS_SCOPE,
    }, () => {
      const runtimeError = chrome?.runtime?.lastError;
      if (runtimeError) {
...
function clearPageScopedChromeProxySettings() {
  const proxySettings = chrome?.proxy?.settings;
...
function validatePageScopedProxyControlAfterApply(details = {}, entry = null) {
...
async function applyPageScopedProxyFromUrl(proxyUrl = '') {
  const entry = parsePageScopedProxyUrl(proxyUrl);
  if (!entry) {
    return null;
  }
  const previousProxySettings = await getPageScopedChromeProxySettings({ incognito: false }).catch(() => ({}));
  const previousAuthEntry = typeof currentIpProxyAuthEntry === 'undefined'
    ? null
    : (currentIpProxyAuthEntry ? { ...currentIpProxyAuthEntry } : null);
  const fixedProxyConfig = buildPageScopedFixedProxyConfig(entry);
  if (!fixedProxyConfig) {
    throw new Error('流程页面代理配置不完整，无法生成 fixed_servers 规则。');
  }
  ...
  try {
    ...
    await setPageScopedChromeProxySettings(fixedProxyConfig);
    const appliedSettings = await getPageScopedChromeProxySettings({ incognito: false }).catch(() => null);
    const takeoverCheck = validatePageScopedProxyControlAfterApply(appliedSettings || {}, entry);
    if (!takeoverCheck?.ok) {
      throw new Error(takeoverCheck.message || '流程页面代理接管校验失败。');
    }
  } catch (error) {
    if (typeof currentIpProxyAuthEntry !== 'undefined') {
    ...
    try {
      ...
      if (restoreValue && restoreValue.mode) {
        await setPageScopedChromeProxySettings(restoreValue);
      } else {
        await clearPageScopedChromeProxySettings();
      }
    } catch {
  ...
  return {
    applied: true,
    entry,
    displayName: describePageScopedProxyEntry(entry),
    previousProxySettings,
    previousAuthEntry,
  };
}

async function restorePageScopedProxySnapshot(snapshot = null) {
  if (!snapshot?.applied) {
    return;
  }
  if (typeof currentIpProxyAuthEntry !== 'undefined') {
...
function isPageScopedProxyFlowActive() {
  return pageScopedProxyFlowDepth > 0;
}

function shouldUsePageScopedProxyForNode(nodeId = '', state = {}) {
  const normalizedNodeId = String(nodeId || '').trim();
  if (!normalizedNodeId || PAGE_SCOPED_PROXY_EXCLUDED_NODE_IDS.has(normalizedNodeId)) {
    return false;
  }
  return !isPageScopedProxyFlowActive()
    && Boolean(state?.pageScopedProxyEnabled)
    && Boolean(String(state?.pageScopedProxyUrl || '').trim());
}

async function withPageScopedProxyForNode(nodeId = '', state = {}, runner = async () => {}) {
  const normalizedNodeId = String(nodeId || '').trim();
  if (!shouldUsePageScopedProxyForNode(normalizedNodeId, state)) {
    return runner();
  }
  if (pageScopedProxyDepth > 0) {
    pageScopedProxyDepth += 1;
    try {
      return await runner();
    } finally {
      pageScopedProxyDepth = Math.max(0, pageScopedProxyDepth - 1);
    }
  }

  pageScopedProxyDepth = 1;
  const proxyUrl = String(state?.pageScopedProxyUrl || '').trim();
  try {
    pageScopedProxySnapshot = await applyPageScopedProxyFromUrl(proxyUrl);
    const displayName = String(pageScopedProxySnapshot?.displayName || proxyUrl).trim();
    if (displayName) {
      await addLog(`流程页面代理已启用：${displayName}（当前节点：${normalizedNodeId}）。`, 'info', { nodeId: normalizedNodeId });
    }
    return await runner();
  } finally {
    pageScopedProxyDepth = 0;
    const snapshot = pageScopedProxySnapshot;
    pageScopedProxySnapshot = null;
    if (snapshot?.applied) {
      try {
        await restorePageScopedProxySnapshot(snapshot);
        await addLog(`流程页面代理已释放（当前节点：${normalizedNodeId}）。`, 'info', { nodeId: normalizedNodeId });
      } catch (restoreError) {
        await addLog(`流程页面代理释放失败：${restoreError?.message || String(restoreError || '未知错误')}`, 'warn', { nodeId: normalizedNodeId });
      }
    }
  }
}

async function withPageScopedProxyForFlow(state = {}, runner = async () => {}) {
  const proxyUrl = String(state?.pageScopedProxyUrl || '').trim();
  if (!Boolean(state?.pageScopedProxyEnabled) || !proxyUrl) {
    return runner();
  }

  if (pageScopedProxyFlowDepth > 0) {
    pageScopedProxyFlowDepth += 1;
    try {
      return await runner();
    } finally {
      pageScopedProxyFlowDepth = Math.max(0, pageScopedProxyFlowDepth - 1);
    }
  }

  pageScopedProxyFlowDepth = 1;
  try {
    pageScopedProxyFlowSnapshot = await applyPageScopedProxyFromUrl(proxyUrl);
    if (pageScopedProxyFlowSnapshot) {
      pageScopedProxyFlowSnapshot = {
        ...pageScopedProxyFlowSnapshot,
        proxyUrl,
      };
    }
    const displayName = String(pageScopedProxyFlowSnapshot?.displayName || proxyUrl).trim();
    if (displayName) {
      await addLog(`流程页面代理已启用：${displayName}`, 'info');
    }
    return await runner();
  } finally {
    pageScopedProxyFlowDepth = 0;
    const snapshot = pageScopedProxyFlowSnapshot;
    pageScopedProxyFlowSnapshot = null;
    if (snapshot?.applied) {
      try {
        await restorePageScopedProxySnapshot(snapshot);
        const displayName = String(snapshot?.displayName || proxyUrl).trim();
        if (displayName) {
          await addLog(`流程页面代理已释放：${displayName}`, 'info');
        }
      } catch (restoreError) {
        await addLog(`流程页面代理释放失败：${restoreError?.message || String(restoreError || '未知错误')}`, 'warn');
      }
    }
  }
}

async function withPageScopedProxyBypassForNode(nodeId = '', runner = async () => {}) {
  const normalizedNodeId = String(nodeId || '').trim();
  const snapshot = pageScopedProxyFlowSnapshot;
  if (!normalizedNodeId || !PAGE_SCOPED_PROXY_EXCLUDED_NODE_IDS.has(normalizedNodeId) || !snapshot?.applied) {
    return runner();
  }

  const proxyUrl = String(snapshot?.proxyUrl || '').trim();
  if (!proxyUrl) {
    return runner();
  }
  ...
  try {
    ...
    try {
      const reapplied = await applyPageScopedProxyFromUrl(proxyUrl);
      pageScopedProxyFlowSnapshot = reapplied
        ? { ...reapplied, proxyUrl }
        : null;
    } catch (reapplyError) {
      await addLog(`流程页面代理在节点 ${normalizedNodeId} 后重启失败：${reapplyError?.message || String(reapplyError || '未知错误')}`, 'warn', { nodeId: normalizedNodeId });
    }
  }
}

async function testPageScopedProxy(options = {}) {
  const proxyUrl = String(options?.proxyUrl || '').trim();
  if (!proxyUrl) {
    throw new Error('请先填写流程页面代理地址。');
  }
  const snapshot = await applyPageScopedProxyFromUrl(proxyUrl);
  ...
  try {
    ...
    if (!exitIp) {
      const diagnostics = typeof buildProbeDiagnosticsSummary === 'function'
        ? buildProbeDiagnosticsSummary(probeDiagnostics, 4)
        : probeDiagnostics.slice(0, 4).join(' | ');
      throw new Error(diagnostics ? `未检测到代理出口 IP。诊断：${diagnostics}` : '未检测到代理出口 IP。');
    }
    ...
    if (typeof detectIpProxyTargetReachabilityByPageContext === 'function') {
      reachability = await detectIpProxyTargetReachabilityByPageContext({
        ...
        targetReachabilityEndpoints: ['https://chatgpt.com/'],
      }).catch((error) => {
    ...
    if (reachability?.reachable === false && reachability?.skipped !== true) {
      const failureMessage = typeof buildTargetReachabilityFailureMessage === 'function'
        ? buildTargetReachabilityFailureMessage({ exitIp, exitRegion }, reachability)
        : `已检测到出口 IP ${exitIp}${exitRegion ? ` [${exitRegion}]` : ''}，但 chatgpt.com 不可达。`;
      throw new Error(failureMessage);
    }

    const diagnostics = typeof buildProbeDiagnosticsSummary === 'function'
      ? buildProbeDiagnosticsSummary([...probeDiagnostics, ...targetDiagnostics], 4)
      : [...probeDiagnostics, ...targetDiagnostics].slice(0, 4).join(' | ');
    return {
      ok: true,
      proxyDisplayName: String(snapshot?.displayName || describePageScopedProxyEntry(snapshot?.entry) || '').trim(),
      exitIp,
      exitRegion,
      exitSource: String(exit?.source || '').trim(),
      exitEndpoint: String(exit?.endpoint || '').trim(),
      targetEndpoint: String(reachability?.endpoint || 'https://chatgpt.com/').trim(),
      diagnostics,
    };
  } finally {
    if (snapshot?.applied) {
      await restorePageScopedProxySnapshot(snapshot).catch(() => {});
    }
  }
}
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        if (isPageScopedProxyFlowActive() && PAGE_SCOPED_PROXY_EXCLUDED_NODE_IDS.has(normalizedNodeId)) {
          await withPageScopedProxyBypassForNode(normalizedNodeId, () => activeStepRegistry.executeNode(normalizedNodeId, executionPayload));
        } else {
          await activeStepRegistry.executeNode(normalizedNodeId, executionPayload);
        }
...
async function runAutoSequenceFromNode(startNodeId, context = {}) {
  ...
  return withPageScopedProxyForFlow(state, () => runAutoSequenceFromNodeGraph(normalizedStartNodeId, context));
}
...
const messageRouter = self.MultiPageBackgroundMessageRouter?.createMessageRouter({
  ...
  testPageScopedProxy: (...args) => testPageScopedProxy(...args),
  fetchGeneratedEmail,
