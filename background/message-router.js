  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'TEST_PAGE_SCOPED_PROXY': {
          const state = await getState();
          if (isAutoRunLockedState(state)) {
            throw new Error('自动流程运行中，当前不能测试流程页面代理。');
          }
          if (typeof testPageScopedProxy !== 'function') {
            throw new Error('流程页面代理测试能力尚未接入。');
          }
          const result = await testPageScopedProxy(message.payload || {});
          return {
            ok: true,
            proxyDisplayName: String(result?.proxyDisplayName || '').trim(),
            exitIp: String(result?.exitIp || '').trim(),
